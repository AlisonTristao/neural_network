...
x_valid = 'data/humano_valid/pictures'
y_valid = 'data/humano_valid/input_.csv'

imagens_valid= load_images(x_valid,1)
input_valid, speed = load_csv(y_valid, 1)
# Load the trained model
model = load_model('model_trained.keras', custom_objects={'CustomAccuracy': CustomAccuracy})

imagens_valid = np.expand_dims(imagens_valid, axis=0) 
# Evaluate the model
loss, accuracy = model.evaluate([imagens_valid,speed], input_valid, verbose=0)
